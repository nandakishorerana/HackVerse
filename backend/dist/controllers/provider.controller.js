...
export const getAllProviders = catchAsync(async (req, res) => {
  ...
  // Filter by city
  if (city) {
    query['serviceArea.cities'] = { $regex RegExp(city as string, 'i') };
  }
  ...
  // Filter by rating
  if (minRating) {
    query.rating = { $gte(minRating as string) };
  }

  // Filter by hourly rate
  if (maxPrice) {
    query.hourlyRate = { $lte(maxPrice as string) };
  }
 ...
 * @access  Public
 */
export const getProviderById = catchAsync(async (req, res, next) => {
  const { id } = req.params;
 ...
 * @access  Public
 */
export const searchProviders = catchAsync(async (req, res, next) => {
  ...
  const totalCount = await ServiceProvider.countDocuments({
    services,
    'serviceArea.cities': { $regex RegExp(city as string, 'i') },
    isVerified,
    isAvailable
  });
 ...
 * @access  Public
 */
export const getTopProviders = catchAsync(async (req, res) => {
 ...
 * @access  Private
 */
export const registerProvider = catchAsync(async (req, res, next) => {
  const {
    services,
    experience,
    hourlyRate,
  ...
  const existingProvider = await ServiceProvider.findOne({ user._id });
  if (existingProvider) {
  ...
  if (services && services.length > 0) {
    const validServices = await Service.find({ _id: { $in }, isActive });
  ...
  successResponse(res, 'Provider profile created successfully', { provider }, 201);
});
 ...
 * @access  Private
 */
export const updateProviderProfile = catchAsync(async (req, res, next) => {
  const { id } = req.params;
  const user = req.user!;
  ...
  successResponse(res, 'Provider profile updated successfully', { provider });
});

/**
 * @desc    Get provider reviews
 * @route   GET /api/v1/providers//reviews
 * @access  Public
 */
export const getProviderReviews = catchAsync(async (req, res, next) => {
  const { id } = req.params;
 ...
 * @route   POST /api/v1/providers//portfolio
 * @access  Private
 */
export const addPortfolioItem = catchAsync(async (req, res, next) => {
  ...
  successResponse(res, 'Portfolio item added successfully', {
    portfolioItem.portfolio[provider.portfolio.length - 1]
  });
});

/**
 * @desc    Update availability
 * @route   PUT /api/v1/providers//availability
 * @access  Private
 */
 ...
 * @route   PATCH /api/v1/providers//toggle-availability
 * @access  Private
 */
 ...
 * @route   PUT /api/v1/providers//verify
 * @access  Private/Admin
 */
export const verifyProvider = catchAsync(async (req, res, next) => {
  const { id } = req.params;
...
export const getProviderStats = catchAsync(async (req, res) => {
  ...
  const topProviders = await ServiceProvider.find({ isVerified, isAvailable })
 ...
 */
export const getMyProviderProfile = catchAsync(async (req, res, next) => {
  const user = req.user!;
 ...
 * @route   GET /api/v1/providers//bookings
 * @access  Private
 */
export const getProviderBookings = catchAsync(async (req, res, next) => {
  const { id } = req.params;
  ...
  successResponse(res, 'Provider bookings retrieved successfully', {
    ...
    pagination: {
      currentPage,
      totalPages.ceil(totalCount / limitNum),
      totalItems,
      itemsPerPage
    }
  });
 ...
 * @route   PUT /api/v1/providers//settings
 * @access  Private
 */
export const updateProviderSettings = catchAsync(async (req, res, next) => {
  const { id } = req.params;
  const user = req.user!;
  ...
  successResponse(res, 'Provider settings updated successfully', { provider });
});

/**
 * @desc    Get provider earnings
 * @route   GET /api/v1/providers//earnings
 * @access  Private
 */
export const getProviderEarnings = catchAsync(async (req, res, next) => {
  const { id } = req.params;
  const user = req.user!;
