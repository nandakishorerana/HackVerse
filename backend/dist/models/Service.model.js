...
ServiceSchema.index({ category });
ServiceSchema.index({ isActive });
ServiceSchema.index({ averageRating: -1 });
ServiceSchema.index({ popularity: -1 });
ServiceSchema.index({ basePrice });
ServiceSchema.index({ createdAt: -1 });
ServiceSchema.index({ name: 'text', description: 'text', tags: 'text' });

// Compound indexes
ServiceSchema.index({ category, isActive, averageRating: -1 });
ServiceSchema.index({ isActive, popularity: -1 });
...
ServiceSchema.methods.incrementPopularity = function() {
  return this.updateOne({ $inc: { popularity } });
};
