...
const io = new Server(httpServer, {
  cors: {
    origin.env.FRONTEND_URL || "http://localhost",
    methods: ["GET", "POST"]
  }
});
...
const speedLimiter = slowDown({
  ...
  delayAfter, // allow 50 requests per 15 minutes, then...
  delayMs // begin adding 500ms of delay per request above 50:
});
...
async function startServer() {
  try {
    ...
    httpServer.listen(PORT, () => {
      ...
      logger.info(`API Documentation available at://localhost:${PORT}/api-docs`);
      logger.info(`Health check available at://localhost:${PORT}/health`);
    });
